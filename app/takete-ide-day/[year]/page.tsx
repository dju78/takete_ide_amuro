...
export default async function TaketeIdeDayYearPage({ params }: Props) {
  ...
  const videos = event.media.filter((m) => m.media_type === "video");
      ...
      <Container className="grid gap-12 py-16 lg:grid-cols-[1fr_320px]">
        <div className="flex flex-col gap-12">
          ...
          {(event.chairman || event.guest_information) && (
            <section className="rounded-2xl border border-purple-600/10 bg-white p-6 shadow-xs">
              <h2 className="font-serif text-xl font-bold text-purple-600">Programme & Dignitaries</h2>
              {event.chairman && <p className="mt-2 text-sm text-charcoal/80"><strong>Chairman:</strong> {event.chairman}</p>}
              {event.guest_information && <p className="mt-1 text-sm text-charcoal/80"><strong>Distinguished Guests:</strong> {event.guest_information}</p>}
          ...
          {event.media.length > 0 ? (
            <section className="space-y-8">
                  ...
                  <div className="mt-4 space-y-6">
                    {videos.map((v, i) => (
                      <CommunityVideo
                        key={i}
                        src={v.url}
                        title={v.caption ?? `Takete-Ide Day ${event.year} Video`}
              ...
              {photos.length > 0 && (
                <div>
                  <h2 className="font-serif text-xl font-bold text-purple-600">Photographs & Gallery</h2>
                  <div className="mt-4">
                    <GalleryLightbox
                      items={photos.map((m, i) => ({
                        id: `${event.year}-${i}`,
                        image_url: m.url,
                        alt_text: m.caption ?? `Takete-Ide Day ${event.year}`,
                        caption: m.caption,
                        category: "Event",
                        title: null,
          ...
          {event.speeches.length > 0 && (
            <section>
              <h2 className="flex items-center gap-2 font-serif text-xl font-bold text-purple-600">
                <Mic className="h-5 w-5" aria-hidden="true" /> Addresses & Remarks
              </h2>
              <div className="mt-4 space-y-4">
                {event.speeches.map((s, i) => (
                  <div key={i} className="rounded-xl border border-purple-600/10 bg-white p-5 shadow-xs">
                    <p className="font-semibold text-purple-600">{s.speaker}</p>
                    {s.title && <p className="text-sm text-gold-700 font-medium">{s.title}</p>}
                    {s.body && <p className="mt-2 text-sm leading-relaxed text-charcoal/80">{s.body}</p>}
                  </div>
                ))}
              </div>
            </section>
          ...
          {event.awards.length > 0 && (
            <section>
              <h2 className="flex items-center gap-2 font-serif text-xl font-bold text-purple-600">
                <Trophy className="h-5 w-5" aria-hidden="true" /> Honours & Award Conferments
              </h2>
              <ul className="mt-4 space-y-3">
                {event.awards.map((a, i) => (
                  <li key={i} className="rounded-xl border border-purple-600/10 bg-white p-5 text-sm shadow-xs">
                    <strong className="text-purple-600 font-serif text-base">{a.recipient}</strong> — <span className="font-semibold text-gold-700">{a.award_title}</span>
                    {a.description && <p className="mt-1.5 leading-relaxed text-charcoal/70">{a.description}</p>}
                  </li>
                ))}
              </ul>
            </section>
            ...
            <section>
              ...
              <div className="mt-4 space-y-3">
                {event.fundraising.map((f, i) => (
                  <div key={i} className="rounded-xl border border-purple-600/10 bg-white p-4 text-sm shadow-xs">
